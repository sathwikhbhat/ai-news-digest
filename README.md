...
AI News Digest is a Spring Boot-based RESTful API that fetches the latest news headlines from NewsAPI.org and provides AI-powered summaries using Google's Gemini API. The application offers both JSON and HTML formatted responses with intelligent caching for optimal performance.
...
- **Multiple Output Formats**: Supports both JSON text summaries and HTML-rendered pages
...
```
**Response**: Complete HTML page with styled news summary
...
4. **AI Processing**: GeminiService sends news data to the Google Gemini API for summarization
5. **Response Formatting**: Based on endpoint, returns either JSON text or HTML-formatted response
6. **Caching**: Response is cached for future requests
7. **Client Response**: Formatted response is returned to the client
...
- [Google Gemini API](https://ai.google.dev/) for AI summarization capabilities
